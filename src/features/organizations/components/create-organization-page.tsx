...
export function CreateOrganizationPage() {
  ...
  const { mutate, isPending, error } = useMutation({
    mutationFn: async () => {
      const { data } = await apiClient.post<CreateOrgResponse>("/organizations", {
        name: name.trim(),
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center gap-6 px-6">
      <div className="text-center">
        <h1 className="text-2xl font-semibold text-foreground">Criar um grupo novo</h1>
        <p className="mt-2 text-muted-foreground">Você vira o líder responsável por ele.</p>
      </div>
